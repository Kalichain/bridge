...
// =============================================================================
// CONTRAT SOURCE - BLOCKCHAIN KALIS GETH (Verrouillage des coins natifs KALIS)
// =============================================================================

contract KalisSourceBridge is Ownable, ReentrancyGuard, Pausable {
    
    // Constantes pour les chaînes
    string public constant CHAIN_ID = "GETH";
    string public constant DESTINATION_CHAIN = "AVAX";
    ...
    uint256 public minBridgeAmount = 0.001 ether; // 0.001 KALIS minimum
    uint256 public maxBridgeAmount = 1000 ether;  // 1000 KALIS maximum
    uint256 public totalLocked;                   // Total verrouillé actuellement
    uint256 public totalFeesCollected;           // Total des frais collectés
    uint256 public bridgeFee = 0.0001 ether;     // Frais de bridge en KALIS
    uint256 public constant NONCE_TIMEOUT = 7 days; // Timeout pour les nonces
    
    // Mappings
    mapping(address => bool) public authorizedRelayers;
    mapping(bytes32 => bool) public processedUnlocks; // Utilise des nonces globaux
    mapping(uint256 => uint256) public nonceTimestamps; // Timestamps des nonces locaux
    mapping(address => uint256) public userNonces; // Nonce par utilisateur
    ...
    function _generateSourceGlobalNonce(uint256 sourceNonce) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(DESTINATION_CHAIN, sourceNonce));
    }
    
    /**
     * @notice Verrouille les coins natifs KALIS pour le bridge vers Avalanche
     * @param recipient Adresse destinataire sur la chaîne de destination
     */
    function lockCoins(address recipient) 
        external 
        payable 
        nonReentrant 
        whenNotPaused 
    ...
    /**
     * @notice Déverrouille les coins suite à un bridge de retour depuis Avalanche
     * @param recipient Destinataire des coins
     * @param amount Montant à déverrouiller
     * @param sourceNonce Nonce de la transaction source (depuis Avalanche)
        ...
        crossChainNonceNotExpired(sourceTimestamp)
    {
        ...
        require(!processedUnlocks[globalSourceNonce], "Transaction already processed");
        require(address(this).balance >= amount, "Insufficient contract balance");
        require(totalLocked >= amount, "Amount exceeds total locked");
        
        processedUnlocks[globalSourceNonce] = true;
        totalLocked -= amount;
    ...
    /**
     * @notice Permet de recevoir des KALIS natifs (financement contrôlé)
     */
    ...
    /**
     * @notice Obtient les fonds disponibles pour withdrawal
     */
    ...
    function isGlobalNonceProcessed(bytes32 globalNonce) external view returns (bool) {
        return processedUnlocks[globalNonce];
    }
